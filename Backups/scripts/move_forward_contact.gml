/* move_forward(speed, direction, parent_solid,
 *              conditional_collisions)
 *
 * This script handles forward/backward motion.
 */

var spe, dir, par_sol, cond_col, spr_w, spr_h, loop1, loops2,
    max_dist, dir_inc;
...
if !place_free_cust(x, y, par_sol, cond_col) {return -2};
...
if spe != 0 
   {
     ...
     repeat (loops1) 
        {
          ...
          spe -= new_spe;
          xtar1 = x + lengthdir_x(new_spe, dir);
          ytar1 = y + lengthdir_y(new_spe, dir);
          ...
          /*for (i2 = 0; i2 < loops2; i2 += 1) 
             {
               ...
               new_spe2 = new_spe;
               //new_spe2 *= cos(degtorad(dir_offset)); // Slow down when set off course
               dir1 = dir + dir_offset;
     ...
     if (x == xorig && y == yorig)
       return 0; // failed to move at all
   }
